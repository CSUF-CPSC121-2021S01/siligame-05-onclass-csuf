...
void Game::UpdateScreen() {
  ...
  if (!HasLost()) {
    screen.DrawText(0, 0, std::to_string(score_), 15,
                    graphics::Color(255, 0, 0));
  } else {
    screen.DrawText(screen.GetWidth() / 2, screen.GetHeight() / 2, "Game Over",
                    25, graphics::Color(255, 0, 0));
  }
}
